        ...
        while not shutdown.is_set():
            try:
                if cap is None or not cap.isOpened():
                    ...
                    if not cap.isOpened():
                        logger.warning(f"Failed to connect to stream for camera {camera_config['camera_id']}")
                        time.sleep(reconnect_delay)
                        reconnect_delay = min(reconnect_delay * 1.5, Config.RECONNECT_DELAY_MAX)
                        continue
                    else:
                        logger.info(f"Successfully connected to stream for camera {camera_config['camera_id']}")
                        reconnect_delay = Config.RECONNECT_DELAY_BASE
                ...
                if not ret:
                    logger.warning(f"Frame capture failed for camera {camera_config['camera_id']}")
                    cap.release()
                    cap = None
                    continue
...
class VideoPipeline:
    ...
    def __init__(self, camera_configs: List[dict]):
        ...
        # Flag to indicate when models are loaded
