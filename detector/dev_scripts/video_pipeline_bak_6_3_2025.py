...
class Config:
    ...
    DROP_FRAME = 2  # Frame dropping configuration (1 = no drop, 2 = every second frame, etc.)
    MAX_RECONNECT_ATTEMPTS = 5  # Max attempts to reconnect to the stream
    LOGGING_INTERVAL = 10  # Interval for logging queue sizes and stream info in seconds
    CREATE_VISUALIZATION_FRAMES = False  # Flag to toggle visualization frame creation
    EVIDENCE_IMAGES_DIR = 'evidence_images'  # Directory to save evidence images
    PUSH_DATA = True  # Flag to control data pushing to the database
...
class VideoPipeline:
    ...
    def _setup_shared_state(self):
        ...
        self.frame_queues = {cfg['camera_id']: mp.Queue(maxsize=500) for cfg in self.camera_configs}
    ...
    def start(self):
        ...
        try:
            ...
            while not self.global_shutdown.is_set():
                ...
                self._log_queue_sizes()  # Log queue sizes at regular intervals
                time.sleep(1)
    ...
    def capture_worker(self, rtsp_url: str, camera_id: str, roi_points: np.ndarray):
        ...
        while not self.global_shutdown.is_set():
            try:
                ...
                self.log("debug", f"Reconnecting in {sleep_time}s (attempt {reconnect_attempts})")
        ...
        # Ensure to signal stream end to processing workers
        try:
    ...
    def _should_drop_frame(self) -> bool:
        """Determine if the current frame should be dropped based on the drop frame configuration"""
        return np.random.randint(1, Config.DROP_FRAME + 1) != 1
    ...
    def processing_worker(self, camera_id: str, roi_points: np.ndarray, test_mode: bool):
        ...
        model = YOLO('/Workspace/sg/Mycodes/Repos/Container_OCR/models/Container_Detector_V1.pt').to(self._get_device())
        ocr_engine = RapidOCR()
        consecutive_empty = 0
        best_detection_id = None  # Track the best detection ID
            ...
            while not self.global_shutdown.is_set():
                try:
                    ...
                    if processed.get('best_detection'):
                        ...
                        # Save visualization frame if a new best detection is found and the flag is set
                        if Config.CREATE_VISUALIZATION_FRAMES:
                        ...
                        # Save evidence image whenever a new best detection is found
                        evidence_filename = os.path.join(Config.EVIDENCE_IMAGES_DIR, f"{camera_id}_evidence_{timestamp.strftime('%Y%m%d_%H%M%S')}.png")
                        cv2.imwrite(evidence_filename, frame)  # Save the original frame as an evidence image
                        self.log("info", f"Saved evidence image to {evidence_filename}")

                        # Check for best detection and push to database if necessary
                        best_detection = processed['best_detection']
                        if best_detection['track_id'] != best_detection_id:
                            self.log("debug", f"Best detection: {best_detection}")
                            if 'track_id' not in best_detection or 'ocr_text' not in best_detection:
                                self.log("error", f"Missing keys in best_detection: {best_detection}")
                                continue
                            self.log("debug", f"Timestamp type: {type(timestamp)}, value: {timestamp}")
                            record = {
                                'datetime': timestamp,
                                'ocr_output': best_detection.get('ocr_text', ''),
                                'camera_id': camera_id,
                                'image_path': evidence_filename,
                                'track_id': str(best_detection['track_id'])  # Ensure track_id is a string
                            }
                            self.log("debug", f"Record to insert: {record}")
                            if Config.PUSH_DATA:
                                # Check if a record for this track_id already exists
                                print(best_detection['track_id'])
                                existing_record = self.db_manager.get_record_by_track_id(int(best_detection['track_id']))
                                if existing_record is None:
                                    self.log('warning', f'No existing record found for track_id: {best_detection["track_id"]}')
                                    # Insert new record
                                    self.db_manager.insert_record(record)  # Insert the new record
                                    print("Inserted new record for track_id:", best_detection["track_id"])
                                else:
                                    # Update existing record
                                    self.db_manager.update_record(existing_record['id'], record)
                            best_detection_id = best_detection['track_id']

                            

                except mp.queues.Empty:
                    consecutive_empty += 1
                    if test_mode and consecutive_empty >= Config.MAX_CONSECUTIVE_EMPTY:
                        self.log("debug", f"No frames for {Config.MAX_CONSECUTIVE_EMPTY} checks. Closing worker.")
                        break
                    continue
    ...
    def _log_queue_sizes(self):
        """Log the sizes of the frame queues and other key information"""
        if self.test_mode:
            for camera_id, queue in self.frame_queues.items():
                self.log("info", f"Queue size for {camera_id}: {queue.qsize()}")
                # Log additional camera information
                cfg = next((cfg for cfg in self.camera_configs if cfg['camera_id'] == camera_id), None)
                if cfg:
                    self.log("info", f"Camera {camera_id} - RTSP URL: {cfg['rtsp_url']}, Location: {cfg['location']}")
            time.sleep(Config.LOGGING_INTERVAL)

    def _push_to_database(self, record: dict):
        """Push the detection data to the database."""
        existing_record = self.db_manager.get_record_by_track_id(record['track_id'])
        if existing_record:
            self.db_manager.update_record(existing_record['id'], record)
        else:
...
if __name__ == "__main__":
    ...
    try:
        camera_configs = [
            {
                'rtsp_url': 'rtsp://10.8.0.9:8554/stream101',
                'camera_id': 'CAM_1',
                'location': 'Entrance',
                'roi_points': np.array([[273,1072],[533,841],[1409,807],[1856,1066]], np.int32)
            },
            {
                'rtsp_url': 'rtsp://10.8.0.9:8554/stream102',
                'camera_id': 'CAM_2',
                'location': 'Entrance',
                'roi_points': np.array([[273,1072],[533,841],[1409,807],[1856,1066]], np.int32)
            }
        ...
        pipeline = VideoPipeline(camera_configs, test_mode=False)
        pipeline.start()
