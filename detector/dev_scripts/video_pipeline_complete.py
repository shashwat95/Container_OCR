...
class VideoPipeline:
    def __init__(self, camera_configs, test_mode=False):
        ...
        # Initialize the OCR engine (consider moving this to capture_stream if not thread-safe)
    ...
    def capture_stream(self, rtsp_url, camera_id):
        ...
        Each thread will have its own instance of the OCR engine.
        """
        # Initialize a new OCR engine for this thread
        ocr_engine = RapidOCR()
    ...
    def run_detection(self, frame, roi, frame_id, camera_id, vis_frame=None):
        ...
        if best_detection is not None:
            ...
            # Crop the best detection from the frame
            ...
            if cropped_image.size > 0:  # Check if the cropped area is valid
                ...
                # Run OCR on the image data using the instance variable
                ...
                # Overlay the OCR output below the cropped image in the visualization
                ocr_output_text = f"OCR: {result}";
                # Set the position for the text to be on the right side
                text_x = vis_frame.shape[1] - 10 - cv2.getTextSize(ocr_output_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0][0];
                text_y = 130;  # Adjust this value as needed for vertical spacing
